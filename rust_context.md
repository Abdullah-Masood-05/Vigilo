...
> **Note on section numbering.** §21 (a from-scratch camera capture via
> `ccap-rs`, evaluated and rejected — see its own branch, `ccap-capture`)
> was never merged into this branch or into `main`. It is skipped here
> rather than renumbered, so the section number stays a stable reference
> to the same content across every branch that carries it.
